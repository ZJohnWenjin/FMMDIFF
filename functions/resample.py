...
class LossAwareSampler(ScheduleSampler):
    def update_with_local_losses(self, local_ts, local_losses):
        batch_sizes = [x for x in range(local_ts.size(0))]
        max_bs = max(batch_sizes)

        timestep_batches = [local_ts[bs].item() for bs in batch_sizes]
        loss_batches = [local_losses[bs].item() for bs in batch_sizes]
        self.update_with_all_losses(timestep_batches, loss_batches)
