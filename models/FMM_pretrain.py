        ...
        for epoch in range(1, self.cfg.train.epochs + 1):
            ...
            for iter, (inputs_vol_stack) in enumerate(data_loader):
                ...
                while index_slice < len(inputs_vol):
                    ...
                    if index_slice == 96 and iter % 20 == 0:
                        image = (output[-1]).float().squeeze().detach().cpu().numpy()
                        image = (image * 255).astype(np.uint8)
                        image = np.clip(image, 0, 255)

                        inputs = (inputs[-1]).float().squeeze().detach().cpu().numpy()
                        inputs = (inputs * 255).astype(np.uint8)
                        inputs = np.clip(inputs, 0, 255)

                        concatenated = np.concatenate((image, inputs), axis=1)
                        cv.imwrite(
                            "./results/{}_{}output.png".format(epoch,iter), concatenated)
            ...
            for iter, (inputs_vol_stack) in enumerate(data_loader):
                ...
                while index_slice < len(label_vol):
                    ...
                    if index_slice == 96 and iter % 20 == 0:
                        ...
                        inputs = (label[-1]).float().squeeze().detach().cpu().numpy()
                        inputs = (inputs * 255).astype(np.uint8)
                        inputs = np.clip(inputs, 0, 255)
